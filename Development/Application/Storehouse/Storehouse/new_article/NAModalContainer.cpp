...
bool NAModalContainer::CheckEmpty()
{
	...
	(
		...
		m_naDlg.m_comboType.GetCurSel() == -1							||
		m_naDlg.m_comboSubtype.GetCurSel() == -1						||
		m_naDlg.m_comboFirm.GetCurSel() == -1							||
		m_naDlg.m_comboUnit.GetCurSel() == -1							||
		( m_naDlg.m_comboType.GetCurSel() == 0		&& !m_naDlg.GetDlgItem(IDC_NA_EDIT_TYPE)->GetWindowTextLength())		||
		( m_naDlg.m_comboSubtype.GetCurSel() == 0	&& !m_naDlg.GetDlgItem(IDC_NA_EDIT_SUBTYPE)->GetWindowTextLength())		||
		( m_naDlg.m_comboFirm.GetCurSel() == 0		&& !m_naDlg.GetDlgItem(IDC_NA_EDIT_FIRM)->GetWindowTextLength())		||
		( m_naDlg.m_comboUnit.GetCurSel() == 0		&& !m_naDlg.GetDlgItem(IDC_NA_EDIT_UNIT)->GetWindowTextLength())
	)
		return true;
