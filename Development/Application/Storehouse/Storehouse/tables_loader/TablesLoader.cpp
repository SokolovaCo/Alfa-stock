...
template <class lF>
void TableLoader::LoadData(CComboBox *cb, tstring tableName, table::header_type ht, lF loadFunc, bool loadOther)
{
	if(cb != NULL)
	{
		cb->ResetContent();
...
void TableLoader::LoadTableArticle(CComboBox *cb, bool loadOther)
{
	...
	LoadData(
		...
		{	
			...
			if(cb != NULL && !m_tableSubtype.empty() && !m_tableFirm.empty())
			{
				...
				cStr.Format(_T("%s  %s  %s  %gр."), tmp1, tmp2, m_tableArticle.back().name.data(), m_tableArticle.back().price);
				cb->AddString(cStr);
			}
		},
		loadOther);
...
void TableLoader::LoadTableSupplier(CComboBox *cb, bool loadOther)
{
...
void TableLoader::LoadTableRecipient(CComboBox *cb, bool loadOther)
{
...
void TableLoader::LoadTableDelivery(CComboBox *cb)
{
	m_tableDelivery.clear();
	...
	LoadData(
		...
		[&](CComboBox *cb, table::row_type r)
	{	
		m_tableDelivery.push_back( DSTable(r[0].any_value< long>(), r[1].any_value< boost::posix_time::ptime >(), _ttoi(r[2].any_value<tstring>().data()), r[3].any_value< long >()) );
// 		if(cb != NULL)
// 			cb->AddString( tstring( r[1].any_value< tstring >() + _T(" ") + r[2].any_value< tstring >() + _T(" ") + r[3].any_value< tstring >() ).data() );
	});
}
...
void TableLoader::LoadTableShipment(CComboBox *cb)
{
	m_tableShipment.clear();
	...
	LoadData(
		...
		[&](CComboBox *cb, table::row_type r)
	{	
		m_tableShipment.push_back( DSTable(r[0].any_value< long>(), r[1].any_value< boost::posix_time::ptime >(), _ttoi(r[2].any_value<tstring>().data()), r[3].any_value< long >()) );
